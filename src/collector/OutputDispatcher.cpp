...
{
    ideoengine::base::NetAddress destAddress = executorPosition.GetManager();
    ...
    if ( destIdentifier == selfIdentifier ) {
        ...
        std::shared_ptr<ideoengine::collector::TaskQueue> taskQueue = _selfTasks[boltIndex];
        ...
        (const util::SocketError& socketError) {
            ideoengine::message::Command command(ideoengine::message::Command::Type::SendTuple);
            ...
            try {
                commandClient->SendCommand(command,
                    [destIdentifier, this](const ideoengine::message::Response& response,
        ...
        const std::string& fieldValue, OutputDispatcher::AskFieldCallback callback)
{
    _presidentClient->Connect([taskPathName, fieldValue, callback, this](const message::CommandError&) {
        ideoengine::message::Command command(ideoengine::message::Command::Type::AskField);
        ...
        _presidentClient->SendCommand(command,
                [callback](const ideoengine::message::Response& response, const message::CommandError& error) {
            if ( error.GetType() != message::CommandError::Type::NoError ) {
                LOG(LOG_ERROR) << error.what();
                return;
            }
